...
export function ProfessionalButton({
  ...
  const variantClasses = {
    primary:
      "bg-gradient-to-r from-primary to-accent text-primary-foreground shadow-lg hover:shadow-xl hover:opacity-90 border-0 hover:text-primary-foreground overflow-hidden",
    secondary:
      "bg-card/80 backdrop-blur-sm border border-primary/20 text-foreground hover:border-primary/40 hover:bg-card/90 hover:text-foreground shadow-lg hover:shadow-xl overflow-hidden",
    outline:
      "bg-transparent border border-primary/20 text-foreground hover:border-primary/40 hover:bg-primary/5 hover:text-foreground shadow-md hover:shadow-lg overflow-hidden",
  };
      ...
      {iconPosition === "right" && icon && (
        <span
          className={`relative z-10 group-hover:translate-x-0.5 transition-transform ${
            variant === "primary" ? "text-shadow-sm" : ""
          }`}
        >
          {icon}
        </span>
    ...
    >
      {asChild ? (
        <Button
          ...
          {...props}
        >
          <span className="flex items-center gap-2 justify-center w-full h-full relative z-10">
            {content}
          </span>
        </Button>
      ) : (
